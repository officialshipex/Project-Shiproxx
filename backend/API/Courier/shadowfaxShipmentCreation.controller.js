  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      if (isTransient && attempt < maxRetries) {
        console.warn(`[API Shadowfax createShipment] Write conflict on attempt ${attempt}. Retrying in ${50 * attempt}ms...`);
        await new Promise((resolve) => setTimeout(resolve, 50 * attempt));
        continue;
      }

      console.error("Shadowfax Creation Error:", error.response?.data || error.message);
      return {
        success: false,
        message: error.response?.data?.errors ? JSON.stringify(error.response.data.errors) : (error.response?.data?.message || error.message || "Error creating Shadowfax shipment"),
        error: error.message,
      };
    }
  }
