...
const createSmartshipShipment = async ({
  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      if (respData?.errors) {
        let validationErrorMsg = "Smartship returned validation errors";
        try {
          if (typeof respData.errors === "string") {
            validationErrorMsg = respData.errors;
          } else if (Array.isArray(respData.errors)) {
            validationErrorMsg = respData.errors.join(", ");
          } else if (typeof respData.errors === "object") {
            const details = [];
            for (const key in respData.errors) {
              const errVal = respData.errors[key];
              if (typeof errVal === "string") {
                details.push(errVal);
              } else if (Array.isArray(errVal)) {
                details.push(...errVal);
              } else if (typeof errVal === "object" && errVal !== null) {
                details.push(JSON.stringify(errVal));
              }
            }
            if (details.length > 0) {
              validationErrorMsg = details.join(", ");
            }
          }
        } catch (_) {}
      ...
      if (isTransient && attempt < maxRetries) {
        console.warn(`[API Smartship createShipment] Write conflict on attempt ${attempt}. Retrying in ${50 * attempt}ms...`);
        await new Promise((resolve) => setTimeout(resolve, 50 * attempt));
        continue;
      }

      console.error("Smartship Shipment Error:", error?.response?.data || error.message);

      return {
        success: false,
        message: error.message || "Failed to register Smartship order",
